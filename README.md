...
[BilibiliCrawler 前瞻计划](docs/FORWARD_PLAN.md)。已完成任务的验收与发布证据保留在对应版本清单中，
历史版本计划仅保留对应阶段记录，不再追加新任务。
...
- 发布前仍须完成 `docs/RELEASE_3.5.0.md` 中的真机验收（含四条安装器清理验收）；完成后再填写发布日期并创建标签。
