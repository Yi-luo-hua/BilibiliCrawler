...
| 优先级 | 任务 | 状态 | 完成标准 |
|---|---|---|---|
| P2 | 新增分析模块自定义文本模块 | 待真机验收 | 自定义模块可保存并参与分析，结果进入界面与报告，未启用时输出与实施前一致 |
...
步骤 1–8 已实现并通过门禁：Python 归一化、提示词分隔、按 id 合并、报告快照与服务链路，Tauri
`UiConfig` 与白名单往返，桌面端模块编辑与结果渲染，以及 Python / Rust / TypeScript 三侧回归。
剩余验收标准的前四条需要真实窗口操作，自动化测试不替代。

两处与本文其他段落不一致、按代码为准的地方：

- 数据契约段写的 `user_config.json`，实际是 `UiConfig` 持久化的 `<user_data>/config/ui.json`；
