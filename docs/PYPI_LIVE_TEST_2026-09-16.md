...
| # | 问题 | 状态 |
|---|---|---|
| 1 | CLI/MCP 无法登录，地域地图必空 | 已修：新增 `BILIBILI_COOKIE` / `--cookie`；属地全空时任务只给一条 warning（爬取阶段已说明则分析阶段不再重复），报告空段落改为与登录方式无关的说明文字 |
...
| 5 | 多批深度分析用「；」硬接 | 已修：改为按批分段落，去重，不再出现 `。；`；段落标注的是真实批次号 |
| 6 | 0 评论被判为链接问题 | 已修：到达该判断即说明目标已解析，统一给出「目标可访问但没有评论」 |
| 7 | 无法识别的输入留下空壳 run | 已修：受理阶段返回 `INVALID_INPUT`，不创建 run |
| 8 | `serverInfo.version` 为空 | 已修：懒解析版本号后传给 `MCPServer` |
| 9 | BUSY 文案重复 task_id | 已修 |
| 10 | 进度通知停在 14% | 已修：终态补一条 100%，并与终态在同一把锁内写入 |
...
回归：`python -m unittest discover -s tests` 由 366 增至 404 个测试全绿（审查修正后为 406）；新增
...
- **#6 在 `570567a` 里只对视频生效。** 判断依据是 `task.target`，而它只由视频解析器填写，
  动态和专栏走不同形状的响应、保持为空——恰好漏掉了实测里那条空动态。进一步看发现
  「请检查链接」分支根本不可达：`crawl_comments` 的所有失败路径都抛 `CrawlError`，
  在这之前就已结算，能走到空结果判断就说明目标已解析。于是删掉这个分支，而不是再加一个
  恒为真的标志。原测试之所以通过，是因为 `FakeCrawler` 总是填写 `target_info`——替身
  比真实爬虫宽容。用一条老的、评论数为 0 的转发动态实测：`570567a` 仍提示「请检查链接」，
  修复后提示「目标可以访问」，两次日志都是「动态详情解析成功」「第 1 页评论为空」。
- **新测试解析到了仓库的 `analysis-runs`。** `LiveLoopbackTests` 里的 `RunStore()` 没有隔离
  目录，不写 run 但会解析并探测该目录，破坏了 #53 建立的隔离。run 计数发现不了，改为插桩
  记录 `RunStore.root`：`570567a` 有 1 个实例落到仓库目录，修复后为 0。
- **批次编号不可信。** 不只是去重后重编号（`[A, A, C]` 把第 3 批标成第 2 批）：某批没返回
  该字段时根本不入列，位置在去重前就已经和批次对不上。现在批次号随文本一起携带。

两处逻辑修复都验证过「回退即变红」。测试 404 → 406。

### 合并前复核的修正（`ea8663b`）

复核又发现两处 P3，修复后随 #54 合并：

- **属地提示重复且会自相矛盾。** `crawl-and-analyze` 在爬取阶段和分析阶段各加一条 warning；
  配置了过期 Cookie 时，一条说「会话已失效」，另一条说「通常是匿名爬取」。分析层还与桌面端
  共用，桌面导出的报告会让扫码登录的用户去「设置 BILIBILI_COOKIE」。现在分析层只给与登录
  方式无关的提示，由 AgentService 按调用方替换：爬取阶段已说明则丢弃，单独分析只补设置方法，
  桌面端保持原样。原测试用的是不会发出该 warning 的假分析器，新测试改走真实 `analyze()`，
  只替换 LLM 调用。
- **终态 100% 通知可能丢失。** 终态在锁内发布、通知在出锁后入队；MCP 轮询先取队列再读状态，
  落在两者之间就会看到任务结束却没有 100%。改为在同一把锁内入队。

新增 5 个测试，撤掉修复后其中 4 个变红，另有 2 个既有测试因文案调整随之更新。测试 406 → 411。
