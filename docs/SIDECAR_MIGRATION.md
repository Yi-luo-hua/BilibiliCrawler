...
| | |
|---|---|
| 状态 | 阶段 1–2 已合并；阶段 3 在 PR #9；阶段 4（评论爬取）、阶段 5（精确 `source == "comments"` 的分析）与阶段 6 自动化跨进程验证已完成。**Tauri 真机完整链路仍待发版前手工验证** |
| 前置 | PR #5（输出目录策略统一）、PR #7（行为基线）、PR #8（策略与请求参数分离）均已合并；阶段 4–5 以 PR #9 为基线 |
