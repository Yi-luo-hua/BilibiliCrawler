...
> 2026-08-31 状态说明：v3.3.0 已公开发布。下文保留原始发布准备清单与验收记录，未勾选项不代表当前发布状态。后续执行顺序与进度统一见 [v3.3.0 后续实施计划](POST_3.3.0_PLAN.md)。
...
- [ ] 安装后解析开始菜单和桌面 `BilibiliCrawler.lnk`，确认 `TargetPath` 与 `WorkingDirectory` 指向注册表 `InstallLocation` 下的 v3.3.0 主程序；`IconLocation` 可以显式指向该程序，也可以是 Windows 表示“使用目标默认图标”的 `,0`，但不得指向旧安装。必须从快捷方式启动验收，不能只直接运行 exe。
- [ ] 在当前 Windows 缩放下实机检查运行中任务栏图标并保存截图，同时解析 ICO/EXE 资源确认包含 16/24/32 像素层而非只有单一 16×16 图标；若验收环境可用，再补充 100% / 125% / 150% 矩阵。升级后还要验证快捷方式目标与 Explorer 图标缓存刷新。
