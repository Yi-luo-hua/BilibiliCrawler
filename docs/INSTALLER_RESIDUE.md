...
**问题 1 最终采用第四节的选项 (a)：存在遗留数据就整次跳过清理。**

中间一版曾试图两全：枚举 `_internal` 逐条删除、按名排除那两个目录，从而让清理对所有用户都生效。
它在 review 中被判为 P1 数据丢失风险，核实成立——为了枚举前确认无占用，该版本先把 `_internal`
重命名到临时名再改回，**用户数据在这个窗口里位于第二个名字下**；若进程在窗口内中断，数据留在临时
目录，而后续安装会重建 `_internal`，下一次升级就会把那个临时目录连同数据一起删除。概率低，但
不可恢复。

清掉几个陈旧 DLL 不值得任何可能丢失用户 run 的路径。因此判定改为：`_internal` 下存在
`analysis-runs` 或 `analysis-assets` 时，**钩子什么都不做**，与今天行为完全一致，清理留给这些数据
被迁移之后的某次升级。

代价要说清楚：迁移是复制而非删除，旧目录不会自行消失，所以**这批用户实际上不会获得清理**。这正是
第四节评估选项 (a) 时接受的限制。判据是"旧目录是否存在"而非"是否已迁移"——迁移成功后源目录依然
保留，所以受影响的是所有从 v3.2.0 及更早升上来、`_internal` 下仍有这两个目录的机器；其余所有用户
正常获得清理。

守卫通过之后，`_internal` 里就只剩程序文件，因此重命名探测不再需要"改回"这一步：**重命名本身就是
删除的第一步**——把旧产物移到临时名后直接 `RMDir /r`，没有可被中断的回程窗口，中断残留的临时目录
也只含程序文件，下次运行清掉即可。
