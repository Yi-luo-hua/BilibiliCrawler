...
class RunManagementTests(McpServerTestCase):
    ...
    async def test_delete_run_rejects_non_positive_prune_to(self) -> None:
        self.install_service()
        async with self.client() as client:
            for value in (0, -1):
                with self.subTest(prune_to=value):
                    result = await client.call_tool("delete_run", {"run_id": "", "prune_to": value})
                    self.assertTrue(result.is_error)
                    text = " ".join(getattr(item, "text", "") for item in result.content)
