...
from bilibili_crawler.service.models import ErrorCode, RunStatus, TaskSnapshot
...
def analysis_recovery_hint(snapshot: TaskSnapshot, *, cli: bool = False) -> str:
    ...
    command = (f"python -m bilibili_crawler analyze-run {snapshot.run_id}" if cli else
